...
def new_user():
    ...
    string2 = request.form.get('image_encoding')
    
    table = 'unKnown User'
    data = {'id': user_id, 'Image': string1, 'image_encoding': string2}
    print(data)
    result, error = supabase_client.table(table).insert(data).execute()
...
def get_image():
    ...
    column_names = ['id','Image','image_encoding']
...
def add_user():
    ...
    Image=Image[9:-1]
    Image=Image[2:]
    image_encoding = str(my_list[-2])
    image_encoding = image_encoding[18:-3]
    image_encoding = image_encoding[2:]
    ...
    table = 'Known User'
    data = {'id': u_id, 'Image': Image,'Details':details , 'face_encoding': image_encoding,'Name':name}
